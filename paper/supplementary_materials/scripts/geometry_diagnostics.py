...
All three read the same (class_means, precision) pair that the audited classifier repository's
...
# the audited classifier repository is not an installed package. Its root is located via _repo_paths
# (marker-file search, layout-agnostic) rather than a hardcoded parent-count,
# since  has been deployed both as a the audited classifier repository sibling and nested
...
def compute_geometry_diagnostics(
    ...
    """
    Single entry point. Fits the audited classifier repository's Mahalanobis parameters on
