...
Per docs/experiment_contract.md (E1): takes a single, explicitly-named
...
Loader construction mirrors the audited classifier repository's own
the canonical OOD evaluation script, function _build_isic_train_loader exactly
...
(docs/threats_to_validity.md #2), so the embeddings E1 measures
...
the canonical OOD evaluation script's _collect_baseline_features / _collect_csg_features
...
# layout-agnostic) rather than a hardcoded parent-count --  has been
# deployed both as a the audited classifier repository sibling and nested inside it (see _repo_paths.py).
...
_debug(f"the audited classifier repository root resolved: {_CLASSIFIER_REPO}")
if str(_CLASSIFIER_REPO) not in sys.path:
    sys.path.insert(0, str(_CLASSIFIER_REPO))

_debug("importing the audited classifier repository src modules (this pulls in pytorch_lightning, torchmetrics)...")
...
_debug("the audited classifier repository src modules imported OK")
...
def _build_isic_train_loader(dm: SkinDataModule, batch_size: int, num_workers: int) -> DataLoader:
    """Mirrors the canonical OOD evaluation script, function _build_isic_train_loader exactly."""
...
def load_and_extract(checkpoint: Path, method: str, loader: DataLoader, device) -> tuple[np.ndarray, np.ndarray]:
    ...
    if method == "effb3_single":
        ...
        # (the upstream scripts directory, run_effb3_control.py:229) -- unlike CSGLiteLightning,
