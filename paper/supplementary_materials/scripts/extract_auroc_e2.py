...
Refactored from the audited classifier repository's the canonical OOD evaluation script, the
...
# the audited classifier repository's root is located via _repo_paths (marker-file search,
...
from _repo_paths import find_classifier_root  # noqa: E402

_CLASSIFIER_REPO = find_classifier_root(__file__)
if str(_CLASSIFIER_REPO) not in sys.path:
    sys.path.insert(0, str(_CLASSIFIER_REPO))
...
NUM_CLASSES = 8  # the audited classifier repository's fixed 8-class ISIC label space (src/datasets/constants.py)
REG_EPS = 1e-5  # deliberately NOT the canonical OOD evaluation script's 1e-3 -- matches E1's geometry metrics instead, per open_questions.md Q5
GLOBAL_SEED = 42  # the canonical OOD evaluation script's own fixed seed, independent of the checkpoint's training seed
...
def _require_explicit_file(path_str: str) -> Path:
    ...
    if not p.is_file():
        raise ValueError(
            ...
            "the canonical OOD evaluation script's own _find_ckpt has the identical bug)."
        )
    return p


def _build_isic_train_loader(dm: SkinDataModule, batch_size: int, num_workers: int) -> DataLoader:
    """Byte-for-byte the same construction as extract_embeddings_e1.py's / the canonical OOD evaluation script's."""
...
def load_model(checkpoint: Path, method: str, device):
    ...
    the canonical OOD evaluation script's own efficiency (it loads each checkpoint once and
    ...
    if method == "effb3_single":
        ...
        # (the upstream scripts directory, run_effb3_control.py:229 is the one real
...
def compute_mahalanobis_scores(
    ...
    """
    Mirrors the canonical OOD evaluation script, function _compute_scores's Mahalanobis branch
...
def auroc_fpr95_from_scores(s_id: np.ndarray, s_ood: np.ndarray) -> tuple[float, float]:
    ...
    the canonical OOD evaluation script, not reinterpreted."""
...
def main():
    ...
    parser.add_argument("--metadata_csv", default=str(_CLASSIFIER_REPO / "data" / "master_metadata_lesion_only_soft.csv"))
    parser.add_argument("--batch_size", type=int, default=128)  # matches the canonical OOD evaluation script's own default
    parser.add_argument("--num_workers", type=int, default=8)   # matches the canonical OOD evaluation script's own default (E1 used 4)
