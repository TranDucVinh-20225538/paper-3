...
- **Dimensionality-reduction protocol** for applying the Mardia-kurtosis metric to $z_{context}$/`backbone_raw`, if that's ever attempted despite the scope caution noted in §3 for Family-C tests at higher dimensionality.
- **The E2 association test** (correlation, regression, or otherwise) between these geometry metrics and Mahalanobis AUROC — deferred to `SPEC.md` §7's open item, informed by but not resolved by this document.
