...
from _repo_paths import find_classifier_root, find_second_classifier_root

_CLASSIFIER_REPO = find_classifier_root(__file__)
_DST_ROOT = find_second_classifier_root(__file__)


def _load_module_from_path(module_name: str, file_path: Path):
    """the audited classifier repository and the second classifier repository both use the generic top-level package name `src`, so a plain
...
# the audited classifier repository's ood_metrics.py imports standalone (only torch/numpy/sklearn, no pytorch_lightning).
_csg_ood_metrics = _load_module_from_path("csg_ood_metrics", _CLASSIFIER_REPO / "src" / "utils" / "ood_metrics.py")
ood_metrics = _csg_ood_metrics

# the second classifier repository's ood_vim_react.py must be loaded before scoring.py, which does `from .ood_vim_react
...
# with its relative import satisfied by placing the second classifier repository's `src` package on sys.path ONLY for this
# one import, then removing it immediately so it can't collide with the audited classifier repository's `src` afterward.
...
def resolve_checkpoint(rung: str, seed: int, cached_checkpoint_path: str) -> Path:
    ...
    under the audited classifier repository's own checkpoints/csg_lite/{rung}_s{seed}/ instead:
    ...
    resolved = _CLASSIFIER_REPO / "checkpoints" / "csg_lite" / f"{rung}_s{seed}" / basename
...
def energy_scores(logits: np.ndarray) -> np.ndarray:
    """Higher = more OOD-like. the second classifier repository's OODScorer.score_energy returns the opposite
...
def auroc_fpr95_from_scores(s_id: np.ndarray, s_ood: np.ndarray) -> tuple[float, float]:
    ...
    the audited classifier repository's ood_metrics)."""
