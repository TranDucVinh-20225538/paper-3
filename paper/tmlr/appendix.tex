...
This appendix holds the pipeline diagram and the full metric definitions that
Section~\ref{sec:methods} states in summary form. Nothing here differs from what
was computed; it is the same protocol at greater length.

\input{moved/study_design_figure}

\input{moved/geometry_metrics}

\section{Supplementary results}
\label{app:results-extra}

Section~\ref{sec:geometry-auroc} reports Mahalanobis AUROC by rung and its
association with each geometry metric; Table~\ref{tab:mahalanobis-auroc} and
Figure~\ref{fig:geometry-vs-auroc} give those results in full.

\input{moved/results_auroc_floats}

\subsection{Two candidate explanations for the distance gap}
\label{app:norm-majority}

Section~\ref{sec:systematic-lower} reports that median Mahalanobis distance is
lower for PAD-UFES than for ISIC-test at every checkpoint, and notes that two
obvious explanations do not account for it on their own. Those two analyses are
reported here.

\input{moved/results_norm_majority}

\subsection{Per-rung view of the five scorer variants}
\label{app:five-scorers-figure}

Figure~\ref{fig:five-scorers} plots the values tabulated in Table~\ref{tab:five-scorers}, Panel~A.

\input{moved/fig5_three_scorers}
...
Mahalanobis AUROC, $n = 13$, tie-free), and its exact null distribution is
defined over all $13!$ rank orderings and computed combinatorially; the
implementation does not sequentially materialize 6.2 billion permutations.
Design~B tests an ordinal dose against a value (a geometry metric,
...
\input{tables/table_power_design}
...
One case does not receive a conventional two-sided BCa interval. Where an observed $\tau$ attains the maximum
...
resampling distribution is one-sided and boundary-constrained by construction.
A conventional two-sided BCa interval would therefore not have its usual
interior-estimate interpretation. That case is reported as a point without such
an interval, and its exact permutation $p$-value is the primary inference.

\input{tables/table_tau_ci}
...
size used in the analysis criterion (Section~\ref{sec:stats}). That value lies below
every $\tau_{\mathrm{crit}}$, so the criterion's conjunction of $|\tau| \geq 0.3$
...
\section{Scorer invariance, sensitivity, and what ``geometry'' can predict}
\label{app:invariance}

This appendix develops the argument summarized in
Section~\ref{sec:invariance-pointer}.

\input{moved/invariance}

\section{Extended related work}
\label{app:related-work}

Section~\ref{sec:related-work} summarizes six adjacent literatures in four
paragraphs. This appendix gives the extended treatment of each.

\input{moved/related_work}

\subsection{Relation to literature, continued}
\label{app:relation-lit}

Section~\ref{sec:relation-lit} returns to the two literatures whose relationship
to these findings is most direct. This subsection covers the remaining ones and
the practitioner implications.

\input{moved/relation_lit}
